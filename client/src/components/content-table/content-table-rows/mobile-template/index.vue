<template>
  <div class="MobileContentTableMobileRows">
    <div v-if="!loadingMarker && rows.length">
      <div
        v-for="(item, index) in rows"
        :key="index"
        class="ContentTableMobile-Elem"
        :class="{
          'ContentTableMobile-Elem_tree': listType === 'tree',
          'ContentTableMobile-Elem_branches': listType === 'branches',
        }"
      >
        <div
          v-for="(value, key, i) in item"
          :key="i"
          class="ContentTableMobile-Item"
          :class="{ 'ContentTableMobile-Item_tree': listType === 'tree' }"
        >
          <div class="" v-if="key === 'folder'">
            <div class="ContentTableMobile-Icon">
            ...
            <router-link class="ContentTableMobile-Link_folder" :to="{ path: `${route.fullPath}/${value}` }">
              {{ value }}
            </router-link>
          </div>

          <div v-else-if="key === 'file'">
            <div class="ContentTableMobile-Icon">
            ...
            <div v-if="value.match(/readme/gi)" class="ContentTableMobile-Icon">
              <svg xmlns="http://www.w3.org/2000/svg" width="10" height="13" viewBox="0 0 10 13" fill="none">
                <path
            ...
            <router-link
              class="ContentTableMobile-Link_file"
              :to="{
          ...
          <router-link
            v-else-if="key === 'hash'"
            class="ContentTableMobile-Link_commit"
            :to="{ path: `${route.fullPath}/${value}` }"
          >
            {{ value }}
          </router-link>

          <div v-else class="ContentTableMobile-Text" :class="{ 'ContentTableMobile-Text_committer': key === 'committer' }">
            {{ value }}
          </div>
        </div>
      </div>
